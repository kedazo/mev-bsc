...
func (s *BundleAPI) CallBundle(ctx context.Context, args CallBundleArgs) (map[string]interface{}, error) {
	...
	for _, tx := range txs {
		...
		// state.Prepare(tx.Hash(), i) ORIG mev-geth <david>
		state.Prepare(rules, from, coinbase, tx.To(), precompiles, tx.AccessList())
...
func (s *BundleAPI) EstimateGasBundle(ctx context.Context, args EstimateGasBundleArgs) (map[string]interface{}, error) {
	...
	for _, txArgs := range args.Txs {
		...
		state.Prepare(rules, *txArgs.From, coinbase, txArgs.To, precompiles, *txArgs.AccessList)
